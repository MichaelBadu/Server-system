...
//It handles the user inputs in the terminal ctrl+c
static void handler(int sig, siginfo_t *siginfo, void *context)
{
//checks and handles error
...
//Receive the user inputs at the client section
...
//Sends the student details to when the client chooses the option
...
//Sends random numbers to the client
void send_random_numbers(int socket){
	int i, k;
	char random_list [10000];
//This make the random numbers generated different
	srand(time(0));
//The function loops through 5 times
	for (i = 0; i <= 4; i++) {
		char str_k[5];
...
//the ensure there is a comma from the start 
			strcat(random_list,",");
			strcat(random_list,str_k);
		}
	}
...
//Sends the content of the uname file to the client
void send_uname_info(int socket){
	struct utsname uts;
//uname gives the details of the OS and sends them to the struct key
	if (uname(&uts) == -1) {
    	perror("uname error");
    	exit(EXIT_FAILURE);
	}
...
//Goes through all the files in the directory and saves them in the files list
...
//Reads the file line by line and saves the files into the file content
